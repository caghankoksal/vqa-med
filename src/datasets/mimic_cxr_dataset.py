...
class MIMICCXR(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.preprocessed:
            folder_path = sample["folder_path"].replace(
                "mimic-cxr-jpg", "mimic-cxr-jpg-resized"
            )
        ...
        if self.image_type == "dcm":
            ...
            for img in dcom_images:
                ...
                if len(img_2d_scaled.shape) == 2:
                    ...
                    img_2d_scaled_process = np.concatenate(
                        [
                ...
                if self.transforms is not None:
                    ...
                    if not self.return_pil:
                        img_2d_scaled_process = T.transforms.ToTensor()(
                            img_2d_scaled_process
                        )
            ...
            images_path = [
                x for x in os.listdir(folder_path) if x.endswith(("jpg", "png"))
            ]
        ...
        if self.only_images is False:
            ...
            eoc_token_id = self.tokenizer.all_special_ids[
                self.tokenizer.all_special_tokens.index("<EOC>")
            ]
            pad_token_id = self.tokenizer.pad_token_id
            targets = torch.cat(
                (input_ids[:, 1:], torch.tensor([pad_token_id]).unsqueeze(1)), dim=1
            )
...
class MIMICCXRDataModule(pl.LightningDataModule):
    ...
    def setup(self):
        ...
        if self.split_path == None:
            self.data_points = self.return_valid_samples(
                self.cxr_dcm_path, self.cxr_jpg_path
            )
            if self.limit_num_samples != None:
                self.data_points = self.data_points[: self.limit_num_samples]

            self.train_split, val = model_selection.train_test_split(
                self.data_points, test_size=0.2
            )
            self.val_split, self.test_split = model_selection.train_test_split(
                val, test_size=0.5
            )
        ...
        self.validation_dataset = MIMICCXR(
            self.val_split,
        ...
        for folder in tqdm(
            ...
            for patient in [
                ...
                for record in [
                    ...
                    path_of_record_folder = os.path.join(
                        main_path, folder, patient, record
                    )
